...
**MANDATORY:** Before presenting any role, you must perform a direct "Live Status" check:
1. **Follow Redirects:** Use web_fetch to hit the final destination URL.
2. **Detect Inactive Markers:** If the page contains "No longer active", "This position has been filled", "Job not found", or redirects to a generic /careers page, discard the role.
3. **Verify Application Form:** Ensure an active "Apply" button or form is present on the page.
4. **Freshness Bias:** Prioritize roles with a confirmed "Posted [X] days ago" stamp over roles with ambiguous dates.
